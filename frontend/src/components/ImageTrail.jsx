...
class ImageTrailVariant1 {
  constructor(container) {
    ...
    const handlePointerMove = ev => { if (ev.type === 'touchmove' && ev.cancelable) ev.preventDefault();
    ...
    const initRender = ev => { if (ev.type === 'touchmove' && ev.cancelable) ev.preventDefault();
      const rect = this.container.getBoundingClientRect();
      this.mousePos = getLocalPointerPos(ev, rect);
  ...
  return (
    <div className="w-full h-full relative z-[100] rounded-lg bg-transparent overflow-visible" ref={containerRef}>
      {items.map((url, i) => (
        ...
        >
          <div
            className="content__img-inner bg-center bg-cover w-[calc(100%+40px)] h-[calc(100%+40px)] absolute top-[-10px] left-[-10px]"
